...
void viewer(FrameBuffer *gfb, int line)
{
	int i, j;
	for(i=0; i<10; i++) {
		char line_a[10];
		sprintf(line_a, "%d", line+i+1);
		drawText(gfb, 190+20, 160+20+35*i, "   ", 0, 0); // line number
		drawText(gfb, 190+20, 160+20+35*i, line_a, 0xFF00FF00, 0);
		
		printf("%d/%d\n", line/10+1, line_num/10+1);
		sprintf(line_a, "%d/%d", line/10+1, line_num/10+1); // page
		drawText(gfb, 790+10+50, 100+10, line_a, 0xFFFFFFFF, 0);

		for(j=0; j<50; j++)
			drawText(gfb, 190+20+30+16*j, 160+20+35*i, "  ", 0, 0); // text
		drawText(gfb, 190+20+30, 160+20+35*i, text[line+i], 0xFFFFFFFF, 0);
	}
}
...
int main(int argc, char* argv[])
{
	...
	while(1)
	{
		...
		strcpy(text[line_num], buff);
		text[line_num][strlen(text[line_num])-1] = '\0';
		printf("%s", text[line_num]);
		line_num++;
	}
	fclose(fd);
	...
	while (!endFlag)
	{
		...
		if (kbhit())
		{
			...
			switch ( ch )
			{
				...
				case 'x': // line down
					line++;
					if(line >= line_num-10) line = line_num-10;
					break;
